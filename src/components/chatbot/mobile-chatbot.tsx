...
const useDraggable = (ref: RefObject<HTMLElement>, handleRef: RefObject<HTMLElement>) => {
    ...
    useEffect(() => {
        // Set initial position for mobile - smaller and positioned better
        const margin = 16;
        const width = 360;
        const height = 520;
        setPosition({ 
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            const width = 360;
            const height = 520;
            const constrainedX = Math.max(margin, Math.min(window.innerWidth - width - margin, newX));
            const constrainedY = Math.max(margin, Math.min(window.innerHeight - height - margin, newY));
        ...
        const handleTouchMove = (e: TouchEvent) => {
            ...
            const newX = e.touches[0].clientX - offset.current.x;
            const newY = e.touches[0].clientY - offset.current.y;
...
export function MobileChatbot() {
  ...
  return (
    <div
      ref={draggableRef}
      className="fixed z-[9999] shadow-2xl rounded-lg"
      style={{ 
        left: `${position.x}px`, 
        top: `${position.y}px`,
        right: 'auto',
    ...
    >
      <ShineBorder
        className="w-[360px] h-[520px] bg-background/80 backdrop-blur-xl"
